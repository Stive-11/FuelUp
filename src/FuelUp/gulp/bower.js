...
gulp.task('bower_dev:build', ['bower:scripts', 'bower:styles']);

gulp.task('bower:scripts', ['bower:partials'], function() {
  return gulp.src([
    path.join(paths.src, '/app/fuelup/**/*.js'),
    path.join(paths.tmp, 'partials', 'templateCacheHtml.js')
  ])
    .pipe($.angularFilesort())
    .pipe($.ngAnnotate())
    .pipe($.concat('fuelup.js'))
...
gulp.task('bower:styles', function() {
  return gulp.src(path.join(paths.src, '/app/fuelup/fuelup.scss'))
